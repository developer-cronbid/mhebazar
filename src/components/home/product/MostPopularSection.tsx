...
export default async function MostPopularSection() {
  ...
  try {
    ...
    if (error.response) {
      console.error(`Most Popular SSR Fetch Error: ${error.response.status} ${error.response.statusText}`);
    } else {
      console.error("Most Popular SSR Fetch Error:", error.message || error);
    }
